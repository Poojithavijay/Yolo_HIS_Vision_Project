...
while True:
	...
	if len(idxs) > 0:
		...
		for i in idxs.flatten():
			...
			cv2.line(frame, (415, 361), (847, 361), (0, 0, 0xFF), 3)
			...
			cv2.putText(frame,"Vehicles in ROI:",(855, 364),cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 0xFF), 2)
			...
			"""
			var = 1
			if (y > 360 and y < 362.5):
			
				f = open("D:\yolo-object-detection\iter.txt", "a")
				f.write("%d\n" %var)
				print("Dump Val:{}".format(var))
				f.close()
				#f = open(r"D:\yolo-object-detection\iter.txt", "r")
				#var1 = f.read()
				var1 = sum([int(s.strip()) for s in open("D:\yolo-object-detection\iter.txt", "r").readlines()])
				print("Read Val:{}".format(var1))
				
				text2 = "{}".format(var1)
				cv2.putText(frame,text2,(1010, 364),cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 0xFF), 2)
				f.close()
